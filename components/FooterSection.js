                ...
                <div className='w-full py-16 grid grid-cols-1 lg:grid-cols-2 gap-16'>
                    ...
                    <div className='flex flex-col gap-6'>
                        ...
                        <p className='text-muted text-sm leading-relaxed max-w-xs'>
                            Subscribe to our newsletter and be the first to discover new arrivals, exclusive offers, and the art of fine fragrance.
                        </p>
                        <div className='flex items-center border border-muted'>
                            <input
                                ...
                                className='bg-transparent px-4 py-2.5 text-sm outline-none flex-1 text-foreground placeholder:text-muted'
                            />
                            <button className='bg-foreground text-background px-4 py-3 hover:opacity-80 transition-opacity'>
                                <Send size={16} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className='grid grid-cols-3 gap-8'>
                        {footerLinks.map((col) => (
                            <div key={col.title} className='flex flex-col gap-4'>
                                <h4 className='font-semibold text-sm tracking-widest uppercase'>{col.title}</h4>
                                <ul className='flex flex-col gap-3'>
                                    {col.links.map((link) => (
                                        <li key={link.label}>
                                            <Link
                                                href={link.href}
                                                className='text-muted text-sm hover:text-foreground transition-colors ease-linear duration-200'
                                            >
                                                {link.label}
                                            </Link>
                                        </li>
            ...
            <section className='border-t border-background py-5'>
                <section className='w-10/12 mx-auto flex items-center gap-5 justify-between max-md:flex-col max-md:'>
                    ...
                    <div className='flex items-center gap-3'>
                        <img src="/amex.svg" alt="American Express" className='w-12 h-auto' />
                        <img src="/master-card.svg" alt="Master Card" className='w-10 h-auto' />
                        <img src="/visa.svg" alt="Visa" className='w-12 h-auto' />
                    </div>
