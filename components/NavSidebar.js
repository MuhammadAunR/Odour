            ...
            <ul className='flex flex-col items-center justify-center left-[-50%] right-[-50%] translate-y-1/2'>
                {navOptions.map((opt, i) => (
                    <motion.li
                        ...
                        viewport={{ once: false }}
                        key={i} className='text-xl w-25 h-14 flex items-center justify-center transition-all ease-linear duration-300 relative group/navOption overflow-hidden'>
                        <Link href={opt.src} className='text-foreground group-hover/navOption:translate-y-10 group-hover/navOption:scale-0 transition-all ease-linear duration-300'>{opt.option}</Link>
