...
const Cart = () => {
  ...
  return (
    <main className="w-full flex">
      <section
        onClick={toggleCart}
        className={`bg-surface/50 backdrop-blur-lg h-screen fixed top-0 z-200 w-full ${isCartOpen ? "block" : "hidden"}`}
      ></section>
      ...
      >
        <div className="flex items-center justify-between px-5 py-6.5 border-b-2 border-accent shrink-0">
          ...
          <span onClick={toggleCart}>
            <X
              size={28}
              className="cursor-pointer hover:rotate-180 hover:text-accent transition-all ease-linear"
            />
          </span>
        </div>
